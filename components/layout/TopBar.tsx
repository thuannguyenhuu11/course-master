  ...
  return (
    <div className="flex justify-between items-center p-4">
      ...
      <div className="max-md:hidden w-[400px] rounded-full flex">
        ...
        <button
          title="button"
          className="bg-[#0099FF] rounded-r-full border-none outline-none cursor-pointer px-4 py-3 hover:bg-[#0099FF]/80"
      ...
      <div className="flex gap-6 items-center">
        <div className="max-sm:hidden flex gap-6">
          {topRoutes.map(route => (
            <Link href={route.path} key={route.path} className="text-sm font-medium hover:text-[#0099FF]">
              {route.label}
            </Link>
          ))}
        </div>
